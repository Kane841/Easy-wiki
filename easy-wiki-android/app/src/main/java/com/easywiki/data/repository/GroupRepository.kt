...
import com.easywiki.model.GroupInviteResponse
...
class GroupRepository(
    ...
    suspend fun createInvite(groupId: Long): Result<GroupInviteResponse> = runCatching {
        val response = api().createInvite(groupId)
        if (response.code != 0 || response.data == null) {
            throw IllegalStateException(response.message.ifBlank { "生成邀请令牌失败" })
        }
        response.data
    }
